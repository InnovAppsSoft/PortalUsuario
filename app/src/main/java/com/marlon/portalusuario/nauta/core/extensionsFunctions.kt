...
fun NautaUser.toEntity(id: Int = 0, password: String): User = User(
    id = id,
    ...
    serviceType = if (this.serviceType.contains("Navegaci\\u00f3n Internacional")) NavigationType.INTERNATIONAL else NavigationType.NATIONAL,
...
fun String.isValidAmountToTransfer(user: UserModel): Boolean = this != "0" && this != "" && this.toFloat() <= user.credit

fun String.formatAsRechargeCode(): String {
    val formattedText = this.replace(" ", "")
    val regex = "(.{4})".toRegex() // crea una expesion regular para dividdir la cadena en grupos de cuatro caracteres
    return formattedText.replace(regex, "$1 ").trim() // reeplaza cada grupo de cuatro caracteres con el mismo grupo seguido de un espacio
}
