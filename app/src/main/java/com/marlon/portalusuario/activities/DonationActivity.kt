...
class DonationActivity : AppCompatActivity() {
    ...
    private fun initUI() {
        ...
        binding.btn2.setOnClickListener {
            ...
            val amount =
                binding.monto2.text.toString().trim { it <= ' ' }.ifBlank {
                    Toast.makeText(this@DonationActivity, ALL_FIELDS_ARE_REQUIRED, Toast.LENGTH_SHORT).show()
                    return@setOnClickListener
                }
            val password =
                binding.pass2.text.toString().trim { it <= ' ' }.ifBlank {
                    Toast.makeText(this@DonationActivity, ALL_FIELDS_ARE_REQUIRED, Toast.LENGTH_SHORT).show()
                    return@setOnClickListener
                }
            launchCall(phoneNumber, amount, password)
        ...
        binding.btn3.setOnClickListener {
            val phoneNumber = "54871663"
            val amount =
                binding.monto3.text.toString().trim { it <= ' ' }.ifBlank {
                    Toast.makeText(this@DonationActivity, ALL_FIELDS_ARE_REQUIRED, Toast.LENGTH_SHORT).show()
                    return@setOnClickListener
                }
            val password =
                binding.pass3.text.toString().trim { it <= ' ' }.ifBlank {
                    Toast.makeText(this@DonationActivity, ALL_FIELDS_ARE_REQUIRED, Toast.LENGTH_SHORT).show()
                    return@setOnClickListener
                }
            launchCall(phoneNumber, amount, password)
